...
def webhook():
    ...
    if "message" in data and "text" in data["message"]:
        ...
        if msg.startswith("/start"):
            ...
            if len(parts) > 1:
                agent_phone = parts[1]
                # Projection `{"_id": 1}` በመጠቀም RAM እንዳይጨናነቅ ተደርጓል
...
def is_winner(card, drawn_numbers):
    ...
    # 1 & 2. የአግድም (Rows) እና የቁመት (Columns) ማረጋገጫ (ፈጣን እንዲሆን በአንድ ሉፕ ተጠቃሏል)
    for i in range(5):
    ...
    if all(card[idx] in drawn_set for idx in [0, 4, 20, 24]): return True
        ...
        if current_status == "lobby":
            ...
            with state_lock:
                if game_state["status"] == "lobby" and len(game_state["players"]) >= 2:
                    ...
                    game_state["timer"] = 30
                    shuffled = []
                    # ተጫዋች ከሌለ ሉፑ ዝም ብሎ በመሽከርከር CPU እንዳያጨናንቅ ማረፊያ ጊዜ
                    time.sleep(2)
                    continue
...
def get_status():
    ...
    # 500 ሰው በተደጋጋሚ ሲጠይቅ ሙሉ ዶክመንት መጫን RAM ስለሚሞላ የባላንስ ፊልድ ብቻ ይነበባል
    user = wallets.find_one({"phone": phone}, {"balance": 1, "_id": 0}) if phone else None
    
    with state_lock:
        p_data = game_state["players"].get(phone, {"cards": {}})
        cards_list = list(p_data["cards"].values())
        
        # 500 ሰው ላይ deepcopy ወይም dict unpack በየሴኮንዱ CPU ስለሚበላ በቀጥታ አዲስ ዲክሽነሪ ማዘጋጀት
        status_copy = {
...
def buy_ticket():
    ...
    # ከዳታቤዝ ብር መቀነስ (የተመረጠ ፊልድ ብቻ Projection በማድረግ አቅሙ ጨምሯል)
